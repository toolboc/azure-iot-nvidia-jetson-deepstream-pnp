...
void DeepstreamInterface_Command_SetDeepStreamConfigCallback(const DIGITALTWIN_CLIENT_COMMAND_REQUEST* commandRequest, DIGITALTWIN_CLIENT_COMMAND_RESPONSE* commandResponse, void* userInterfaceContext)
{
    ...
    if (commandResult == DIGITALTWIN_COMMAND_OK &&
        Deepstream_SerializeSetDeepStreamConfigCommandResponse(&response, responsePayload, MAX_MASSAGE_SIZE))
    {
    ...
    else
    {
        ...
        char* errorResponse = "\"Failed to execute setDeepStreamConfig command\"";
...
static void DeepstreamInterface_TelemetryCallback(DIGITALTWIN_CLIENT_RESULT digitalTwinTelemetryStatus, void* userContextCallback)
{
    ...
    {
        SendTelemetry_Succeeded_Callback("Deepstream", (const char*)userContextCallback);
        LogInfo("DEEPSTREAM_INTERFACE: DigitalTwin successfully delivered telemetry message for <%s>", (const char*)userContextCallback);
    }
    else
    {
        SendTelemetry_Error_Callback("Deepstream", (const char*)userContextCallback);
        LogError("DEEPSTREAM_INTERFACE: DigitalTwin failed to deliver telemetry message for <%s>, error=<%s> ", (const char*)userContextCallback, MU_ENUM_TO_STRING(DIGITALTWIN_CLIENT_RESULT, digitalTwinTelemetryStatus));
    }
}

// SendTelemetry is a helper function which is periodically invoked by the caller to send telemetry
DIGITALTWIN_CLIENT_RESULT DeepstreamInterface_SendTelemetry_Internal(DIGITALTWIN_INTERFACE_CLIENT_HANDLE interfaceHandle, const char* telemetryName, const char* telemetryData)
{
    DIGITALTWIN_CLIENT_RESULT result;

    result = DigitalTwin_InterfaceClient_SendTelemetryAsync(interfaceHandle, telemetryName, telemetryData,
        DeepstreamInterface_TelemetryCallback, (void*)telemetryName);

    if (result != DIGITALTWIN_CLIENT_OK)
    {
        LogError("DEEPSTREAM_INTERFACE:: DigitalTwin_InterfaceClient_SendTelemetryAsync failed for sending %s", telemetryName);
    }
    else
    {
        LogInfo("DEEPSTREAM_INTERFACE:: DigitalTwin_InterfaceClient_SendTelemetryAsync successfully sent %s", telemetryName);
    }
...
DIGITALTWIN_CLIENT_RESULT DeepstreamInterface_Telemetry_SendAll()
{
    ...
    if (((result = DeepstreamInterface_Telemetry_SendDeepstream_status()) != DIGITALTWIN_CLIENT_OK) ||
        ((result = DeepstreamInterface_Telemetry_SendDeepstream_detections()) != DIGITALTWIN_CLIENT_OK) )

    {
        LogError("DEEPSTREAM_INTERFACE: failed to send telemetries");
    }
    else
    {
        LogInfo("DEEPSTREAM_INTERFACE: Queuing of all telemetries to be sent has succeeded");
    }

    return result;
}

DIGITALTWIN_CLIENT_RESULT DeepstreamInterface_Telemetry_SendDeepstream_status()
{
    ...
    if (Deepstream_SerializeDeepstream_statusTelemetry(payloadBuffer, MAX_MASSAGE_SIZE))
    {
        return DeepstreamInterface_SendTelemetry_Internal(appState.interfaceClientHandle, DeepstreamInterface_Deepstream_statusTelemetry, payloadBuffer);
    }
    else
    {
        LogError("DEEPSTREAM_INTERFACE: serialize telemetry message deepstream_status failed.");
        return DIGITALTWIN_CLIENT_ERROR;
    }
}

DIGITALTWIN_CLIENT_RESULT DeepstreamInterface_Telemetry_SendDeepstream_detections()
{
    if (appState.interfaceClientHandle == NULL)
    {
        LogError("DEEPSTREAM_INTERFACE: interfaceClientHandle is required to be initialized before sending telemetries");
    ...
    char payloadBuffer[MAX_MASSAGE_SIZE];
    if (Deepstream_SerializeDeepstream_detectionsTelemetry(payloadBuffer, MAX_MASSAGE_SIZE))
    {
        return DeepstreamInterface_SendTelemetry_Internal(appState.interfaceClientHandle, DeepstreamInterface_Deepstream_detectionsTelemetry, payloadBuffer);
    }
    else
    {
        LogError("DEEPSTREAM_INTERFACE: serialize telemetry message deepstream_detections failed.");
        return DIGITALTWIN_CLIENT_ERROR;
    }
}
...
DIGITALTWIN_INTERFACE_CLIENT_HANDLE DeepstreamInterface_Create()
{
    ...
    else if ((result = DigitalTwin_InterfaceClient_SetCommandsCallback(interfaceHandle, DeepstreamInterface_ProcessCommandUpdate)) != DIGITALTWIN_CLIENT_OK)
    {
